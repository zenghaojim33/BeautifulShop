...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        // BWMMark
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:@"关闭" style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
//            self.title = [NSString stringWithFormat:@"%lu %@", (unsigned long)numberOfPhotos, photosText];
            self.title = [NSString stringWithFormat:@"共%lu张图", (unsigned long)numberOfPhotos];
        }
...
//            self.title = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
            self.title = [NSString stringWithFormat:@"共%lu张 当前第%lu张",(unsigned long)numberOfPhotos, (unsigned long)(_currentPageIndex+1)];
        }
	} else {
		self.title = nil;
	}
